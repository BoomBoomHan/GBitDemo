...
public class ScriptCompileReloadTools
{
    ...
    const string menuRealodDomain = "Tools/Reload %`";
    ...
    private static void OnCompilationStarted(object obj)
    {
        ...
        {
            compileSW.Start();
            Debug.Log("<color=yellow>编译已开始。</color>");
        }
    }
    //结束编译
    private static void OnCompilationFinished(object obj)
    {
        ...
        {
            compileSW.Stop();
            Debug.Log($"<color=yellow>编译成功，耗时:{compileSW.ElapsedMilliseconds} ms</color>");
            compileSW.Reset();
        }
    }
    ...
    private static void OnBeforeAssemblyReload()
    {
        if (IsManualReload)
        {
            Debug.Log("<color=yellow>重载已开始。</color>");
            //记录时间
    ...
    private static void OnAfterAssemblyReload()
    {
        if (IsManualReload)
        {
            ...
            Debug.Log($"<color=yellow>重载完成，耗时:{timeMS} ms</color>");
            LockRealodDomain();
        }
    }
